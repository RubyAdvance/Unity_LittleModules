...
Shader "Spine/Skeleton Tint" {
	Properties {
		[PerRendererData] _GlitterValue("GlitterValue",Float) = 1
	...
	SubShader {
		...
		Pass {
			...
			float4 frag (VertexOutput i) : SV_Target {
				float4 color = _AddColor * _GlitterValue;
				color.a = 1;
				float4 texColor = tex2D(_MainTex, i.uv) * color;
		...
		Pass {
			...
			float4 frag (VertexOutput i) : SV_Target {
				float4 color = _AddColor * _GlitterValue;
				color.a = 1;
				fixed4 texcol = tex2D(_MainTex, i.uvAndAlpha.xy) * color;
